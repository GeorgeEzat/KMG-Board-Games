...
public:
    NUM_X_O_Player (string name, T symbol, vector<int> &available_numbers);
...
public:
    NUM_X_O_Random_Player (T symbol, vector<int> &available_numbers);
...
template <typename T>
NUM_X_O_Player<T>::NUM_X_O_Player(string name, T symbol, vector<int> &available_numbers) : Player<T>(name, symbol) {
...
void NUM_X_O_Player<T>::getmove(int& x, int& y) {
    while (true) {
        ...
        cout << "\nPlease enter your move (x y value) and value separated by spaces (0 0 9): ";
        cin >> x >> y >> mark;
        ...
        if (hasNumber(mark)) {
            ...
            this->symbol = -1; // will not update the board
            break;
        }
    }
}
...
template <typename T>
NUM_X_O_Random_Player<T>::NUM_X_O_Random_Player (T symbol, vector<int> &available_numbers) : RandomPlayer<T>(symbol) {
    Rand_num = available_numbers;
    this->dimension = 3;
...
void NUM_X_O_Random_Player<T>::getmove(int& x, int& y) {
    ...
    this->symbol = Rand_num[rand() % Rand_num.size()];
}
